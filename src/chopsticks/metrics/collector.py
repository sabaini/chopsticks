...
class MetricsCollector:
    ...
    def record_operation(self, metric: OperationMetric):
        ...
        if (datetime.utcnow() - self._window_start).total_seconds() >= self.aggregation_window:
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "overall_performance": {
                "duration_ms": self._compute_statistics(durations).to_dict() if durations else {},
                "throughput_mbps": self._compute_statistics(throughputs).to_dict() if throughputs else {}
            },
    ...
    def export_csv(self, output_path: Path):
        ...
        fieldnames = [
            'operation_id', 'timestamp_start', 'timestamp_end',
            'operation_type', 'workload_type', 'object_key',
            'object_size_bytes', 'duration_ms', 'throughput_mbps',
            'success', 'error_code', 'retry_count', 'driver', 'user_id'
        ]
