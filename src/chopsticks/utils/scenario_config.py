    ...
    if custom_config_path:  # Non-empty string
        ...
        if custom_path.exists():
            try:
                ...
                raise RuntimeError(f"Failed to load scenario config from {custom_config_path}: {e}")
    ...
    if not config:
        ...
        if default_path.exists():
            try:
                ...
                raise RuntimeError(f"Failed to load default scenario config from {default_path}: {e}")
